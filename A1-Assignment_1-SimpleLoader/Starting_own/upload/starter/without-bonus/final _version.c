...
void load_and_run_elf(char **exe)
{
	...
	memcpy(virtual_memory, data_ptr + phdr->p_offset, phdr->p_filesz);

	// 4. Navigate to the entrypoint address into the segment loaded in the memory in above step
	// 5. Typecast the address to that of function pointer matching "_start" method in fib.c.
	int (*_start)() = (int (*)())((virtual_memory) + ((ehdr->e_entry) - (phdr->p_vaddr)));
...
bool elf_file_check(int File, Elf32_Ehdr e_hdr)
{
	bool file_check = null_file_check(File);
	if (file_check)
	{
