...
// }
struct process *dequeue_enqueue_processes(struct process *running, struct process *queue) {
    struct process *current = running;
    struct process *next_process;

    while (current != NULL) {
        next_process = current->next;  // Save the next process for the next iteration

        if (waitpid(current->pid, NULL, WNOHANG) != 0) {  // Process has terminated
            // Log and free the terminated process
            add_to_history(current->pid, current->command, current->wait_time, current->num_slice * TSLICE);
            free(current);
        } else {
            // Re-enqueue still-running process
            queue = enqueue(queue, current);
        }

        current = next_process;  // Move to the next process
    }

    return queue;  // Return the updated queue with remaining processes
}
